 ...
 * @brief Object detection using YOLOv8 with Neurova DNN module
 * 
 * This example demonstrates how to:
 * - Load a YOLOv8 ONNX model
...
void printAvailableBackends() {
    ...
    for (const auto& [backend, target] : backends) {
        ...
        switch (backend) {
            ...
            case nv::dnn::DNN_BACKEND_OPENCV: backendName = "OpenCV"; break;
...
int main(int argc, char** argv) {
    // Configuration
    const std::string modelPath = (argc > 1) ? argv[1] : "yolov8n.onnx";
    ...
    if (!useCuda) {
        net.setPreferableBackend(nv::dnn::DNN_BACKEND_OPENCV);
